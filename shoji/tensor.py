...
class Tensor:
	...
	def __init__(self, dtype: str, dims: Union[Tuple[Union[None, int, str], ...]], inits: Union[List[np.ndarray], np.ndarray] = None, length: int = None) -> None:
		"""
		Args:
		...
		"""
		self.dtype = dtype
		self.length = length  # Length is the actual length as stored (or None if unknown)
		...
		self.assigned_name = ""  # Will be set if the Tensor is read from the db
		...
		if self.inits is None:
			...
			if np.isscalar(self.inits):
				self.inits = np.array(self.inits)
				...
				for i, array in enumerate(self.inits):
					...
					for ix in range(len(shape)):
						if shape[ix] != array.shape[ix]:
							shape[ix] = None
				self.shape = tuple([len(self.inits)] + shape)
	...
	def __repr__(self) -> str:
		return f"<Tensor '{self.dtype}' {self.dims}, {self.length} row{'s' if self.length != 1 else ''}>"
